!function($){
    ...
    MMPaginator.prototype = {
        ...
        , _plain: function(page, totalCount, limit){
            ...
            //
            var $prev = $('<li class="prev"><a><i class="glyphicon glyphicon-backward"></i></a></li>');
            if(page<=1){
            ...
            //
            var $next = $('<li class="next"><a title="下一页"><i class="glyphicon glyphicon-forward"></i></a></li>');
            if(page>=totalPage){
        ...
        , load: function(params){
            ...
            var page = datas[opts.pageParamName];
            if(page === undefined || page === null){
                page = $el.data('page');
            }
            $el.data('page', page);

            var totalCount = datas[opts.totalCountName];
            if(totalCount === undefined){
                totalCount = 0;
            }
            $el.data('totalCount', totalCount);

            var limit = datas[opts.limitParamName];
            if(!limit){
                limit = $limitList.val();
            }
