...
/* 2017-01-06 标注框 */
P.Plot.TipsPlace = function(t) {
    goog.base(this, []), this.type = P.PlotTypes.TIPS_PLACE, this.t = .4, this.fixPointCount = 3, this.setPoints(t)
}, goog.inherits(P.Plot.TipsPlace, ol.geom.Polygon), goog.mixin(P.Plot.TipsPlace.prototype, P.Plot.prototype),
    P.Plot.TipsPlace.prototype.generate = function() {
    var t = this.getPoints();
    if (!(t.length < 2)) {
        if (2 == this.getPointCount()) {
            ...
            //确定第三个点的方位
            var pnt3 = t[2];
            //由于我国的经纬度都是正值，所有暂时没有管南半球的情况
            //求出矩形对角线的直线公式y=kx+b
            //斜率为负的对角线k1
            var k1 = ((ymax - ymin)/(xmin - xmax)).toFixed(12);
            var b1 = (ymax - (ymax - ymin)/(xmin - xmax)*xmin).toFixed(12);
            //斜率为正的对角线k2
            var k2 = ((ymax - ymin)/(xmax - xmin)).toFixed(12);
            var b2 = (ymax - (ymax - ymin)/(xmax - xmin)*xmax).toFixed(12);
            //第三个点在k1上的x,y值
            var x1 = ((parseFloat(pnt3[1]) - b1)/k1).toFixed(12);
            var y1 = k1*parseFloat(pnt3[0]) + parseFloat(b1);
            //第三个点在k2上的x,y值
            var x2 = ((parseFloat(pnt3[1]) - b2)/k2).toFixed(12);
            var y2 = k2*parseFloat(pnt3[0]) + parseFloat(b2);
            //如果第三个点在矩形内,则无视这个点
            if(parseFloat(pnt3[1]) >= y1 && parseFloat(pnt3[1]) >= y2 && parseFloat(pnt3[1]) > ymax){
                //第三个点在上方
                var ln = [xmin + parseFloat(((xmax - xmin)/3).toFixed(12)),ymax];
                var rn = [xmin + parseFloat(((xmax - xmin)/3*2).toFixed(12)),ymax];
                this.setCoordinates([[tl,ln,pnt3,rn,tr, br, bl]]);
            }else if(parseFloat(pnt3[0]) > x1 && parseFloat(pnt3[0]) > x2 && parseFloat(pnt3[0]) > xmax){
                //第三个点在右边
                var dn = [xmax,ymin + parseFloat(((ymax - ymin)/3).toFixed(12))];
                var upn = [xmax,ymin + parseFloat(((ymax - ymin)/3*2).toFixed(12))];
                this.setCoordinates([[tl,tr,upn,pnt3,dn, br, bl]]);
            }else if(parseFloat(pnt3[1]) <= y1 && parseFloat(pnt3[1]) <= y2 && parseFloat(pnt3[1]) < ymin){
                //第三个点在下边
                var ln = [xmin + parseFloat(((xmax - xmin)/3).toFixed(12)),ymin];
                var rn = [xmin + parseFloat(((xmax - xmin)/3*2).toFixed(12)),ymin];
                this.setCoordinates([[tl,tr, br, rn,pnt3,ln , bl]]);
            }else if(parseFloat(pnt3[0]) < x1 && parseFloat(pnt3[0]) < x2 && parseFloat(pnt3[0]) < xmin){
                //第三个点在左边
                var dn = [xmin,ymin + parseFloat(((ymax - ymin)/3).toFixed(12))];
                var upn = [xmin,ymin + parseFloat(((ymax - ymin)/3*2).toFixed(12))];
                this.setCoordinates([[tl,tr, br, bl,dn,pnt3,upn]]);
            }else{
                this.setCoordinates([[tl, tr, br, bl]]);
            }
        }
...
P.Plot.GatheringPlace = function(t) {
    ...
    switch (t) {
        ...
        case P.PlotTypes.TIPS_PLACE:
            return new P.Plot.TipsPlace(o);
    ...
    this.deactivate(), this.deactivateMapTools(), this.map.on("click", this.mapFirstClickHandler, this), this.plotType = t, this.plotParams = o,
    ...
    return this.points.push(t.coordinate), this.plot = P.PlotFactory.createPlot(this.plotType, this.points, this.plotParams), this.feature = new ol.Feature(this.plot),
        /* add by xxx 20161214 由于个人业务,增加feature的属性,plotType表示当前feature的绘制类型,plotParams表示当前feature的功能类型:1为要图标绘,2为文字标绘 */  this.feature.setProperties({"plotType":this.plotType}),this.feature.setProperties({"plotParams":this.plotParams}),
        this.featureSource.addFeature(this.feature), this.map.un("click", this.mapFirstClickHandler, this), this.plot.fixPointCount == this.plot.getPointCount() ? void this.mapDoubleClickHandler(t) : (this.map.on("click", this.mapNextClickHandler, this), this.plot.freehand || this.map.on("dblclick", this.mapDoubleClickHandler, this), void goog.events.listen(this.mapViewport, P.Event.EventType.MOUSEMOVE, this.mapMouseMoveHandler, !1, this))
